const colorFor = (v) => v > 0 ? "#16a34a" : v < 0 ? "#dc2626" : "#374151"; // [cite: 52, 219]
const meterCtx = (v) => v >= 7 ? "green" : v <= 4 ? "red" : ""; // [cite: 37, 191]
...
function renderGlobal(d) {
    ...
    fill.className = `meter-fill ${meterCtx(d.meter)}`; // Restored Color [cite: 191]
    fill.style.width = (d.meter * 10) + "%";
    document.getElementById("globalMeterValue").innerText = Math.round(d.meter); // Integer 

    document.getElementById("globalMarkets").innerHTML = Object.entries(d.indices).map(([k, v]) => `
        <span class="item-wrap">
            <span class="sq ${v.status === 'OPEN' ? 'green' : 'grey'}"></span> ${k} <span style="color:${colorFor(v.change_30m)}">${v.change_30m >= 0 ? '+' : ''}${v.change_30m}%</span> </span>`).join('<span class="gap">|</span>');
}
